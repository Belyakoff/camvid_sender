...
int fill_frame(StreamingContext* decoder, AVFrame* frame, uint8_t* frame_buffer_420p) {

    int width = decoder->width;
    int height = decoder->height;

    int dest_linesize_420p[4] = { width, width / 2, width / 2, 0 };

    frame->data[0] = frame_buffer_420p;
    frame->data[1] = frame_buffer_420p + (width * height);
    frame->data[2] = frame_buffer_420p + (width * height * 5 / 4);

    frame->linesize[0] = dest_linesize_420p[0];
    frame->linesize[1] = dest_linesize_420p[1];
    frame->linesize[2] = dest_linesize_420p[2];
...
int open_media_web(const char* in_filename, AVFormatContext** avfc) {
    ...
    if (!av_input_format) {
        printf("Couldn't find vfwcap input format to get webcam\n");
        return false;
    }
    ...
    av_dict_set(&options, "pix_fmt", "yuyv422", 0);  // yuv422; rgb0
...
int prepare_decoder(StreamingContext* decoder) {
    ...
    for (uint16_t i = 0; i < decoder->avfc->nb_streams; ++i) {  
        ...
        if (av_codec_params->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            int bit_rate = 2 * 1000 * 1000;
            decoder->avfc->bit_rate = bit_rate;
            decoder->avfc->streams[i]->time_base = { 1 , 1500 };
            decoder->avfc->streams[i]->r_frame_rate = { 30, 1 };
            decoder->avfc->streams[i]->avg_frame_rate = { 30, 1 };
            
            decoder->video_avcc->time_base = { 1, 1500 };
            decoder->video_avcc->bit_rate = bit_rate;
            decoder->video_avcc->pix_fmt = AV_PIX_FMT_YUYV422;
...
int prepare_video_encoder(StreamingContext* encoder, AVCodecContext* decoder_ctx, AVRational input_framerate, StreamingParams sp) {
    ...
    encoder->video_avs->time_base = { 1, 1500 }; // av_inv_q(input_framerate);
...
bool video_reader_open(StreamingContext* decoder, StreamingContext* encoder, const char* filename) {
    ...
    if (open_media_web(decoder->filename, &decoder->avfc)) return -1;
    
    if (prepare_decoder(decoder)) return -1;

    char* rtmp_adress = "rtmp://localhost/stream/video";

    char* file_name = "test.mp4";

    avformat_alloc_output_context2(&encoder->avfc, NULL, "flv", rtmp_adress);
    if (!encoder->avfc) {
        printf("could not allocate memory for output format");
        return -1;
    }  else {
        printf("Successfully allocated memory for output format \n");
    }
    ...
    while (av_read_frame(decoder->avfc, decoder->av_packet) >= 0) {
        ...
        if (av_codec_params->codec_type == AVMEDIA_TYPE_VIDEO) {
            ...
            if (response >= 0) {
                ...
                uint8_t* frame_buffer_420p = new uint8_t[width * height * 4];
                if (frame_buffer_420p == NULL)
                {
               ...
               // uint8_t* bufferY = new uint8_t[width * height * 4];
               // uint8_t* bufferU = new uint8_t[width * height * 4];
               // uint8_t* bufferV = new uint8_t[width * height * 4];
               // uint8_t* dest_0[4] = { bufferY, bufferU, bufferV, NULL };
                
                
                yuyv_to_yuv420P(*(decoder->av_frame->data), frame_buffer_420p, width, height, NULL);

                AVFrame* frame = decoder->av_frame;
                fill_frame(decoder, frame, frame_buffer_420p);
 
                encode_video(decoder, encoder, frame);
                ...
                printf("----------------------------");
                printf("non video or audio packet \n" );
